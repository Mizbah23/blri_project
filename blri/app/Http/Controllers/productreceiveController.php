...
//    }
    public function getUsers(){
        ...
        $userData['data'] = tblpurchase::getuserData();
...
//dd($request->all());
        $SupplierID = $request->input('supplierName');
        $Purchase_Date = $request->input('Purchase_Date');
        $OrderNo = $request->input('orderNo');
        $IsDonate = $request->input('IsDonate');
        if( $SupplierID !='' &&  $Purchase_Date !='' &&  $OrderNo !='' &&  $IsDonate !=''){
            $data = array('SupplierID'=>$SupplierID,'Purchase_Date'=>$Purchase_Date,"orderNo"=>$OrderNo,"IsDonate"=>$IsDonate,'Job_By'=>1);

            // Call insertData() method of Page Model
            $value = tblpurchase::insertData($data);
            if($value){
                echo $value;
            }else{
                echo 0;
